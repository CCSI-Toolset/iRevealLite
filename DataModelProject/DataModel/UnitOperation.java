...
public class UnitOperation extends Alias implements Serializable
{
	...
	public void exportACMRom(String fileName)
	{
	...
	public void exportCapeOpenRom(String fileName)
	{
		int i, j, k;
		String fileNameWithExtension;
	...
	public void exportFileForSampling(String fileName)
	{
		...
		String fileNameWithExtension;
		//make sure the fileName contains extension ".in"
		int iPoint = fileName.lastIndexOf(".");
		if (iPoint>=0)
			fileNameWithExtension = fileName.substring(0,iPoint) + ".in";
		else
		...
		try
		{
			...
			Parameter param;
			out.println("Input_Model=CFD");
			out.println("RUN_NAME=default");
			out.println("TSTART=0");
			out.println("TSTOP=0");
			out.println("DatFilePath=default");
			out.println("No_Of_Simulations=" + nSample);
			out.println("No_Of_Iterations=1");
			out.println("Iterations_Name_Starts");
			out.println("default");
			out.println("Iterations_Name_Ends");
			out.println("Input_Parameter_Starts");
			for (i=0; i<ninput; i++)
			{
				param = romInputVector.get(i);
			...
			out.println("Input_Parameter_Ends");
			out.println("Output_Parameter_Starts");
			for (i=0; i<noutput; i++)
			{
				param = yromOutputVector.get(i);
				if (param.getAlias()!=null && !param.getAlias().isEmpty())
			...
			out.println("Output_Parameter_Ends");
			out.close();
		}
		catch(IOException e)
		{
	...
	//Command example: java DataModel.UnitOperation -a user_input.json ACM_code.acmf rom.in
	//Command example: java DataModel.UnitOperation -c user_input.json CapeOpen.output rom.in
	public static void main(String argu[])
	{
		...
		if (i<4)
		{
			System.out.println("Number of argument < 4!");
			return;
		}
		...
		/*commented out for writing JSON file
		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().setPrettyPrinting().create();
		String json = gson.toJson(uo);
		try
		{
			FileWriter out_json = new FileWriter("converted.json");
			out_json.write(json);
			out_json.close();
		}
		catch(IOException e)
		...
		*/
		...
		if (argu[0].equals("-a"))
		{
			uo.exportACMRom(argu[2]);
			uo.exportFileForSampling(argu[3]);
			return;
		}
		if (argu[0].equals("-c"))
		{
			uo.exportCapeOpenRom(argu[2]);
			uo.exportFileForSampling(argu[3]);
			return;
		}
