...
class surrogateMethod(surrogate):
    ...
    def run(self):
        ...
        iREVEALDirFullPath = os.getcwd() + '/' +iREVEALDir
        iREVEALExec = self.options["Exec Path"].value
        ResultsFile = self.options["Results File"].value
        ...
        self.msgQueue.put("Results File Name:   " + ResultsFile)
        self.msgQueue.put("------------------------------------")
