...
{
	printf("iReveal command usage:\n");
	printf("\t-s *.json\tprovide a JSON configuration file to sample input space and create iReveal.csv and a temporary ACM file.\n");
	printf("\t-b\t\tuse iReveal.csv in current working directory to build ROM, finalize ACM file and do cross validation.\n");
}
...
int ProcessJsonAndSampleInputSpace(char* filename)
{
	...
	FILE* pfout = fopen("iReveal.csv","w");
	if (pfin==NULL)
	{
		printf("Unable to open iReveal.io file for reading!\n");
		return -1;
	}
	if (pfout==NULL)
	{
		printf("Unable to open iReveal.csv file for writing!\n");
		return -1;
	}
	//sample input space and write csv file for input and output
	char line[500];		//a line in iReveal.io file
	int i;				//counter
	...
	fclose(pfout);
	printf("Input space is sampled successfully.\nPlease use input data in iReveal.csv to run high-fidelity models and then fill in the output data.\n");
	return 0;
}
...
int BuildKrigingRom()
{
	...
	FILE* pfio = fopen("iReveal.io","r");
	if (pfio==NULL)
	{
		printf("Unable to open iReveal.io file for reading!\n");
		return 1;
	}
	FILE* pfcsv = fopen("iReveal.csv","r");
	if (pfcsv==NULL)
	{
		printf("Unable to open iReveal.csv file for reading!\n");
		return 1;
	}
	//read iReveal.io file
	//rom name
	fscanf(pfio, "%s", line);
	rom_name = line;
	fgets(line, 499, pfio);
	...
	for (icase=0; icase<ncase; icase++)
	{
		...
		for (i=0; i<nx; i++)
		{
			...
			{
				printf("Error in reading iReveal.csv file!\n");
				return 1;
			}
			str1 = str.substr(0,j);
			str = str.substr(j+1);
	...
	FILE* pfcvd = fopen("iReveal_cross_validation.csv","w");
	if (pfcvd==NULL)
	{
		printf("Unable to open iReveal_cross_validation.csv for writting!\n");
		return 1;
	}
	CYROM yrom_new;
