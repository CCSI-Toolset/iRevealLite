...
void CMatrix::Write(FILE* pf)
{
	int iversion = 0;
	bool ballocated = ppa!=NULL;
	fwrite(&iversion,sizeof(int),1,pf);
	fwrite(&nrow,sizeof(int),1,pf);
	fwrite(&ncol,sizeof(int),1,pf);
	fwrite(&ballocated,sizeof(bool),1,pf);
	if (ballocated)
	{
		for (int i=0; i<nrow; i++)
			fwrite(ppa[i],sizeof(T_REAL),ncol,pf);
	}
}

void CMatrix::Read(FILE* pf)
{
	int iversion;
	bool ballocated;
	fread(&iversion,sizeof(int),1,pf);
	fread(&nrow,sizeof(int),1,pf);
	fread(&ncol,sizeof(int),1,pf);
	fread(&ballocated,sizeof(bool),1,pf);
	if (ballocated)
	{
		AllocateMemory();
		for (int i=0; i<nrow; i++)
			fread(ppa[i],sizeof(T_REAL),ncol,pf);
	}
}